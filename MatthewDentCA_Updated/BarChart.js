class BarChart {
	constructor(obj){ //Initialising the constructor within the barchart class. Declaring variables that are called within the file and assigning them an object from the sketch.js file.
		...
		this.xAxisPadding = obj.xAxisPadding;
		this.yAxisPadding = obj.yAxisPadding;
	...
	render(){
		...
		text("Value as " + this.yAxisLabel, this.chartWidth/2 - this.xAxisPadding, this.chartHeight/this.chartHeight - this.xAxisPadding, 200);
		pop();

		text("Value as " + this.xAxisLabel, this.chartWidth/2 - this.yAxisPadding, this.chartHeight/this.chartHeight + this.yAxisPadding, 200);
...
class HorizontalBarChart {
	...
	render(){
		...
		text("Value as " + this.xAxisLabel, this.chartWidth/2 - this.xAxisPadding, this.chartHeight/this.chartHeight - this.xAxisPadding, 300);
		pop();

		text("Value as " + this.yAxisLabel, this.chartWidth/2- this.yAxisPadding, this.chartHeight/this.chartHeight + this.yAxisPadding, 250);
	...
	//creating the number of ticks along the desired axis
		...
		push()
		noStroke();
		translate(1, -gap); //translating the origin of the bars
...
class StackedBarChart {
	...
	render(){
		...
		if(this.stackedPos == "REG"){
			push();//pushing to prevent the rotate function from affecting other parts of code
			rotate(this.axisLabelRotation);//using strings to create the labels for the axis pulling the property name from the object.
			text("Value as " + this.yAxisLabel, this.chartWidth/2 - this.xAxisPadding, this.chartHeight/this.chartHeight - this.yAxisPadding, 250);
			pop();

			text("Value as " + this.xAxisLabel, this.chartWidth/2 - this.xAxisPadding, this.chartHeight/this.chartHeight + this.yAxisPadding, 200);
		}
		...
		for(let i = 0; i < this.yValues.length; i++){
			...
			textFont(fontReg)
			fill(this.labelColour)
			text(this.yValues[i],[i]*(this.chartWidth),this.chartHeight/4)
			fill(this.barColour[i]);
		...
		if (this.stackedPos == "REG"){
			...
			push()
			let xLabels = this.data.map(d => d[this.xValue]); //assigned the mapped values of the property xValue to the variable xLabels
				translate(1,-gap); //translating the origin of the bars by the gap
...
class StackedAvgChart {
	...
	render(){
		...
		textSize(this.titleSize);
		textFont(fontBold);
		text(this.titleLabel + " as percentage of " + this.yValues, 0, -this.chartHeight-this.yAxisPadding, 480);
...
class BubbleChart {
	...
	render(){
		...
		push();//pushing to prevent the rotate function from affecting other parts of code
		rotate(this.axisLabelRotation);//using strings to create the labels for the axis pulling the property name from the object.
		text("Value as " + this.yAxisLabel, this.chartWidth/2 - this.xAxisPadding, this.chartHeight/this.chartHeight - this.xAxisPadding, 350);
		pop();

		text("Value as " + this.xAxisLabel, this.chartWidth/5, this.chartHeight/this.chartHeight + this.yAxisPadding, 300);
